...
class ProcessRegister {
    ...
    public static function registerUser (string $firstname, string $lastname, string $email, string $passport, string $password) {
        ...
        if ($user) {
            ...
            $createUser->setColumn("firstname,lastname,email,passport,password");
            $createUser->setTable("users");
            $createUser->setValue(":firstname, :lastname, :email, :passport, :password");
            $createUser->setData(['firstname'=>$f, 'lastname'=>$l, 'email'=>$e, 'passport'=>$passport, 'password'=>$p]);
